...
function parseVulnValidators {
        ...
        echo "" > /root/tested/namada_all_peers_${agtimestart}.tmp
        agpeers=`cat /root/tested/namada_peers_${agtimestart}.txt`
        IFS=' ' read -ra PEERS <<< "$agpeers"
        for i in "${PEERS[@]}"; do
          echo ${i} >> /root/tested/namada_all_peers_${agtimestart}.tmp
        done
        sort /root/tested/namada_all_peers_${agtimestart}.tmp | uniq > /root/tested/namada_all_peers_${agtimestart}.txt
        rm /root/tested/namada_all_peers_${agtimestart}.tmp
        addr_count=`wc -l /root/tested/namada_all_peers_${agtimestart}.txt | awk '{ print $1 }'`
        echo -e "\n\e[104mCollected" ${addr_count} "addresses\e[0m\n"
        echo -e "\n\e[104mAnalysing collected peers...\e[0m\n"
        agpeers=$(cat /root/tested/namada_all_peers_${agtimestart}.txt | tr '\n' ' ')
        ...
        for i in "${FULL_PEERS[@]}"; do
          ...
          && if [ "$vp" -gt "0" ]; then echo "http://"${i}":"${port} "|" ${mon} "|" ${vp} >> /root/tested/namada_vuln_hosts_${agtimestart}.txt && ((agvuln++)); fi \
          && echo -e "\e[1A\e[KChecked" \
          || echo -e "\e[1A\e[KTimeout" \
          || echo -e "\e[1A\e[KPort" ${port} "closed"
        done
...
function sendRequests {
        ...
        cat /root/tested/namada_vuln_hosts_${agtimestart}.txt
        if (( $agvuln>0 )); then
                ...
                do 
                        ...
                        (curl -s ${addr}${request} | jq . && echo "") >> /dev/null | parallel -j 100;
                        ((agcounter++))
                done
                done < /root/tested/namada_vuln_hosts_${agtimestart}.txt
...
function main {
        ...
        WIDTH=40
        CHOICE_HEIGHT=4
        BACKTITLE="namada Stress Tester"
        TITLE="Menu"
